...
try {
    ...
    $get_body_parts_qry = $conn->query("SELECT tbl_body_parts.part_id as part_number,
                                                            (SELECT COUNT(tbl_body_parts_injured.part_id) FROM tbl_body_parts_injured 
                                                             ...
                                                             WHERE tbl_body_parts_injured.is_deleted = 0 AND tbl_first_aid.is_deleted=0 AND YEAR(tbl_first_aid.date_created) = $year AND part_id = tbl_body_parts.part_id  ) as number_per_part, part_name
                                                        FROM tbl_body_parts 
