...
if (isset($_POST['load_tbt_over_view'])) {
    ...
    $count_tbt_qry = $conn->query("SELECT (COUNT(tbt_id))as total_tbt, COUNT(DISTINCT(location)) AS total_location 
                                            FROM tbl_toolbox_talks 
                                            INNER JOIN tbl_employees ON tbl_toolbox_talks.employee_id = tbl_employees.employee_id
                                            WHERE tbt_type"."$comparator"."$select_type AND tbl_toolbox_talks.is_deleted = 0 AND sub_id = $subscriber_id
                                            AND  date_format(date_conducted, '%Y-%m')  =  date_format('$date', '%Y-%m')");
    $count =  $count_tbt_qry->fetch();

    $total_man_power_qry = $conn->query("SELECT COUNT(tbl_toolbox_talks_participants.employee_id) as total_man_power, COALESCE(SUM(time),0) as total_man_hours  FROM tbl_toolbox_talks 
    ...
    $average_manpower_qry = $conn->query("SELECT COUNT(tbl_toolbox_talks_participants.employee_id) as total_monthly_manpower ,date_conducted  
                                                   FROM tbl_toolbox_talks 
                                                   INNER JOIN tbl_toolbox_talks_participants ON tbl_toolbox_talks.tbt_id = tbl_toolbox_talks_participants.tbt_id
                                                   INNER JOIN tbl_employees ON tbl_toolbox_talks.employee_id = tbl_employees.employee_id  
                                                   WHERE tbt_type "."$comparator"."$select_type AND tbl_toolbox_talks.is_deleted = 0  AND sub_id = $subscriber_id          
...
//    if ($type == 1){
//        $href = 'toolbox_civil.php';
//    }elseif ($type == 2){
//        $href = 'toolbox_electrical.php';
//    }elseif ($type == 3){
//        $href = 'toolbox_mechanical.php';
//    }elseif ($type == 4){
//        $href = 'toolbox_camp.php';
//    }elseif ($type == 5){
//        $href = 'toolbox_office.php';
//    }
    ...
    $edit_tbt_access = $conn->query("SELECT `access_id`, `access_type_id`, `status`, `date_created` FROM `user_access` WHERE `user_id` = $user_id AND access_type_id = 10 AND `status` = 0");

    if ($edit_tbt_access->rowCount() > 0){
       $edit_tbt_access = $edit_tbt_access->fetch();
       $edit_tbt_access_status = $edit_tbt_access['status'];
       if ($edit_tbt_access_status == 1){
           $edit_tbt_disable = 'hidden';
       }
    }else{
       $edit_tbt_disable = 'hidden';
    }
    ...
    $delete_tbt_access = $conn->query("SELECT `access_id`, `access_type_id`, `status`, `date_created` FROM `user_access` WHERE `user_id` = $user_id AND access_type_id = 11 AND `status` = 0");
    if ($delete_tbt_access->rowCount() > 0){
       $delete_tbt_access = $delete_tbt_access->fetch();
       $delete_tbt_access_status = $delete_tbt_access['status'];
       if ($delete_tbt_access_status == 1){
           $delete_tbt_disable = 'disabled';
       }
    }else{
       $delete_tbt_disable = 'disabled';
    }
        ...
        <tr>
            ...
            <td class="text-center">
                <div class="btn-group">
                    ...
                    <a href="toolbox_talks.php?view_attendance=<?=$tbt_id?>&type=<?=$type?>" type="button" class="btn btn-sm btn-primary js-tooltip-enabled" <?=$edit_tbt_disable ?> data-toggle="tooltip" title="Edit Employee" data-original-title="Edit">
                        <i class="fa fa-pencil"></i>
                    </a>
                    <button type="button" class="btn btn-sm btn-danger js-tooltip-enabled"  <?=$delete_tbt_disable ?> id="delete" onclick="delete_attendance(<?=$tbt_id?>)" data-toggle="tooltip" title="Delete Attendance" data-original-title="Delete">
                        <i class="fa fa-times"></i>
                    </button>
                </div>
            </td>
...
if (isset($_POST['upload_file_tbt_id'])){
    $tbt_id = $_POST['upload_file_tbt_id'];
    $date_now = date('Y-m-d H:i:s');
    ...
    try {
        ...
        if(!empty($fileNames)){
            foreach($_FILES['files']['name'] as $key=>$val){
                ...
                $fileName = basename($_FILES['files']['name'][$key]);
                $date1 = date("His"); // for unique file name
                $image = $date1.'-'.$fileName; // not actually image but files, the file accepted now allows other extension unlike before
                ...
                if(in_array($fileType, $allowTypes)){
                    ...
                    if(move_uploaded_file($_FILES["files"]["tmp_name"][$key], $targetFilePath)){
                        // Image db insert sql
                            ...
                            $insert = $conn->prepare("INSERT INTO `tbl_tbt_images`(`img_src`, `date_uploaded`, `tbt_id`, `type`) VALUES (?,?,?,?)");
                            $insert->execute([$image,$date_now,$tbt_id,'completion']);
    //                       Update training status as completed
                            $update_training_status = $conn->prepare("UPDATE `tbl_toolbox_talks` SET `status`= ? WHERE `tbt_id`= ?");
                            $update_training_status->execute([1,$tbt_id]);
                            $conn->commit();
                            echo 'uploaded';
